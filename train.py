...
from Patch_csv_512 import MOTSDataSet as MOTSDataSet_normal
from Patch_csv_512 import MOTSValDataSet_512_center as MOTSValDataSet
...
def get_arguments():
    ...
    parser.add_argument("--trainset_dir", type=str, default='/data2/KPMP_test/train/data_list.csv')

    parser.add_argument("--valset_dir", type=str, default='/data2/KPMP_test/val/data_list.csv')
    ...
    parser.add_argument("--snapshot_dir", type=str, default='/data2/DoDNet/snapshots_2D/for_one_class_Swinunetr/fold1_with_white_scale_normalwhole_1217/')
    ...
    parser.add_argument("--val_folder", type=str, default='val_results/')
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        model = SwinUNETR(img_size=512, in_channels=3, out_channels=2, spatial_dims=2)
        check_wo_gpu = 0
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        output_folder = os.path.join(args.val_folder, str(epoch))
                ...
                df.loc[0] = ['0adh', avg_val_F1[0].item(), avg_val_Dice[0].item(), avg_val_TPR[0].item(),
                             avg_val_PPV[0].item()]
                ################################################################################################################
